...
export const setter = <T, U>(getter: (t: T) => U) => {
  let writes = pipe(TRACEABLE as T, getter, deepEntries)
...
export const pathOnGet = <O extends {[$$path]?: PropertyKey[]}>(o: O): any =>
  new Proxy(o, {
    get: (_, p) =>
      p === $$path
        ? o[$$path] || []
...
export const deepEntries = <T, P extends Path<any>>(
  x: T,
  parentPath?: P,
): DeepEntries<T, P> =>
  !isObject(x) || hasProperty(x, $$path)
    ...
    : (Array.isArray(x)
        ...
        deepEntries(v).map(([s, d]) =>
          tuple([...(parentPath || []), k, ...s], d),
        ),
      )
